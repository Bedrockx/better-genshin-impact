...
public class AutoLeyLineOutcropTask : ISoloTask
{
    ...
    private async Task TryKazuhaCollect(Avatar kazuha)
    {
        ...
        if (combatScenes == null)
        {
            _logger.LogWarning("战后聚集拾取：无法获取队伍场景，跳过");
            return;
        }

        var outcome = await BetterGenshinImpact.GameTask.AutoHoeing.Multiplayer.KazuhaCollectExecutor.RunAsync(
            combatScenes,
            waitSkillCdSeconds: 30,
            postPickupWaitMs: KazuhaPickupPostSkillWaitMs,
            onProgress: null,
            ct: _ct);

        if (!outcome.Success)
        {
            _logger.LogWarning("战后聚集拾取：跳过，原因={Reason}", outcome.SkipReason);
            return;
        }
